...
function Show-Usage {
    Write-Host "Usage: .\tools\launch_preview_fake.ps1 [-PythonPath <python.exe>] [-Help]"
    Write-Host ""
    Write-Host "Starts the local preview backend in fake mode on 127.0.0.1:21520."
...
function Invoke-Preflight {
    ...
    $preflightArgs = @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", $scriptPath, "-Mode", "fake")
    if (-not [string]::IsNullOrWhiteSpace($PythonPath)) {
        $preflightArgs += @("-PythonPath", $PythonPath)
    }
    & powershell @preflightArgs
    if ($LASTEXITCODE -ne 0) {
        Write-Error "Preflight failed. Fix the failed checks above, then rerun launch_preview_fake.ps1."
        exit $LASTEXITCODE
    }
    Write-Host ""
}
...
function Start-BackendFake {
    param([string]$PythonPath)

    $oldRunner = $env:S2PASS_BACKEND_RUNNER
    try {
        $env:S2PASS_BACKEND_RUNNER = "fake"
...
Write-Host "S2Pass local preview (fake mode)"
...
$entries = @()
try {
    Write-Host "Starting backend fake mode on $BackendHost`:$BackendPort..."
    $backend = Start-BackendFake -PythonPath $python
    $entries += [pscustomobject]@{
        name = "backend"
        pid = $backend.Id
        command = "$python -m backend.server --host $BackendHost --port $BackendPort (S2PASS_BACKEND_RUNNER=fake)"
        role = "backend_fake"
    ...
    $health = Wait-HealthMode -ExpectedMode "fake"

    Write-Host ""
    Write-Host "S2Pass Local Preview (fake) is ready."
