...
3. 创建或加入房间。
4. 配置目标游戏端口。
5. 启动游戏并连接 Co-opWinG 显示的本地地址。
6. 如果失败，查看日志、流量状态和 relay 诊断信息。
...
4. Configure the target game port.
5. Start the game and connect to the local address shown by Co-opWinG.
6. If something fails, check logs, traffic counters, and relay diagnostics.
